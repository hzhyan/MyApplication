...
public class GLRenderer implements GLSurfaceView.Renderer {
    ...
    public GLRenderer(Context context) {
        try {
            mModel = new STLReader().parserBinStlInAssets(context, "connorposed.stl");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
...
//        gl.glLoadIdentity();
        ...
        //设置法向量数据源
        gl.glNormalPointer(GL10.GL_FLOAT, 0, mModel.getVnormBuffer());
        // 设置三角形顶点数据源
        gl.glVertexPointer(3, GL10.GL_FLOAT, 0, mModel.getVertBuffer());

        // 绘制三角形
        gl.glDrawArrays(GL10.GL_TRIANGLES, 0, mModel.getFacetCount() * 3);
